...
def createCSV():
    ...
    dataframe = pd.read_csv('Image Sketcher/emotions.txt', sep=",",header=None)
    dfWords = dataframe[0]
    ...
    with open('Image Sketcher/emotionimages.csv', 'w') as f:
        wr = csv.writer(f)
    ...
    print(f"Process scraping took {time.time() - start_time} seconds.")  # End measuring time
...
def createDict():
    emotionColors = dict()
    with open('Image Sketcher/emotions_images_colors.csv') as f:
        fileString = f.read()
...
def generate_and_save_image(client, prompt_text, size_value, model="dall-e-2", quality_value="standard"):
    ...
    save_dir = "Image Sketcher/generated_images"
    ...
    try:
        ...
        if response.data and isinstance(response.data, list):
            ...
            if image_response.status_code == 200:
                ...
                print(f"Process of image generation took {time.time() - start_time} seconds.")  # End measuring time

                print(f"Image successfully generated and saved at {save_path}")
                # Remove the file after processing to avoid reprocessing
                if (mode==2):
                    os.remove("Image Sketcher/node-red-mentalcommand.txt")
                elif (mode==1) :
                     os.remove("Image Sketcher/node-red-emotion.txt")
            else:
